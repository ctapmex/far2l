...
{
  std::unique_lock lock(mtx);
  COLORER_LOG_WARN("[ColorerWorker] Получена новая задача: начать с %", invalid_line);
  this->valid_line = invalid_line - 1;
...
{
  COLORER_LOG_WARN("[Thread] Начало валидации строк.");
...
UnicodeString* ColorerWorker::getLine(size_t lno)
{
  ...
  while ((internal_buffer.empty()) || (buffer_start_line + internal_buffer.size() <= lno)) {
    COLORER_LOG_WARN("[Thread] Буфер пуст. Запрашиваю данные...");
    data_needed = true;
    ...
    data_cv.wait(lock, [this, lno] { return !data_needed || stop_flag || has_new_task; });
